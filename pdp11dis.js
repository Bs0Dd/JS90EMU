...
function FetchWord() {
    ...
    else if ((DISADDR >= ROMSTA) && (DISADDR < (ROM.length+ROMSTA))) {
        word = (((ROM[DISADDR-ROMSTA+1] & 0xFF) << 8) | (ROM[DISADDR-ROMSTA] & 0xFF));
    }
    else if ((ROMT != null) && (DISADDR >= ROTSTA) && (DISADDR < (ROMT.length+ROTSTA))) {
        word = (((ROMT[DISADDR-ROTSTA+1] & 0xFF) << 8) | (ROMT[DISADDR-ROTSTA] & 0xFF));
    }
...
function StoreWord(x) {
    if (((M92_STAT & (1 << 13)) != 0)&&(DISADDR>=0x4000)&&(DISADDR<0x8000)) { // 92 ROM access
        ...
        ROMT[DISADDR-ROTSTA+1] = (x>>8) & 0xFF;
        ROMT[DISADDR-ROTSTA] = x & 0xFF;
    }
    else if ((DISADDR >= RAMSTA) && (DISADDR < (RAM.length+RAMSTA))) {
